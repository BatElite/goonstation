	...
	choose_name(var/retries = 3, var/what_you_are = null, var/default_name = null, var/force_instead = 0)
		...
		for (retries, retries > 0, retries--)
			if(force_instead)
				...
				newname = input(src,"You are a Cyborg. Would you like to change your name to something else?", "Name Change", client?.preferences?.borg_preferred_name ? client.preferences.borg_preferred_name : default_name) as null|text
		...
		switch(severity)
			if(OLD_EX_SEVERITY_1)
				SPAWN(1 DECI SECOND)
					src.gib(1)
				return
			if(OLD_EX_SEVERITY_2) damage = 40
			if(OLD_EX_SEVERITY_3) damage = 20
	...
	*/
	...
	proc/update_bodypart(var/part = "all")
		...
		if (C?.painted)
			...
			var/w = (1.5 - avg / 2) / 3
			var/too_dark = max(0, 0.25 - avg) //up from 0.15 with recolourable borg HUDs (black HUD was too dark and the borgs were getting too dark highlights)
			col[1] += too_dark * 255
			col[2] += too_dark * 255
			col[3] += too_dark * 255
			color_matrix = list(0,0,0,w, 0,0,0,w, 0,0,0,w, 0,0,0,0, col[1]/255, col[2]/255, col[3]/255, -0.3)
			hud.recolour("#[num2hex(col[1],2)][num2hex(col[2],2)][num2hex(col[3],2)]")
		else
			hud.recolour("#FFFFFF") //reset to white
