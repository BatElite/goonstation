...
/obj/storage/secure/closet/medical/medkit
	...
	spawn_contents = list()
	autosorting = FALSE //doesn't look as good on medkits as the custom thing below
	make_my_stuff()
		if (..()) // make_my_stuff is called multiple times due to lazy init, so the parent returns 1 if it actually fired and 0 if it already has
...
/obj/storage/secure/closet/medical/chemical
	...
	spawn_contents = list()
	req_access = list(access_medical_director)
	autosorting = FALSE //bottles look better with the below
	make_my_stuff()
		if (..()) // make_my_stuff is called multiple times due to lazy init, so the parent returns 1 if it actually fired and 0 if it already has
...
/obj/storage/secure/closet/research/chemical
	...
	spawn_contents = list()
	autosorting = FALSE //bottles look better with the stuff below
	make_my_stuff()
		if (..()) // make_my_stuff is called multiple times due to lazy init, so the parent returns 1 if it actually fired and 0 if it already has
