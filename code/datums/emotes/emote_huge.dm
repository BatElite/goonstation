		...
		if (!user.get_organ("butt"))
			...
			if (!fart_on_other)
				switch(rand(1, 42))
					...
					if (21) message = "<B>[user]</B> farts. It smells like George Melons' perfume!"
					...
					if (27) message = "<B>[user]</B> sharts! That's just nasty."
		...
		if (!user.lying)
			...
			for (var/obj/item/grab/G in user.equipped_list(check_for_magtractor = 0))
				...
				if(!length(combatflipped))
					...
					var/do_flip = TRUE
					var/orig_user_flags = user.flags
					var/orig_tmob_flags = tmob.flags
					user.flags |= TABLEPASS
					tmob.flags |= TABLEPASS
					if(!istype(oldloc) || !istype(newloc))
						do_flip = FALSE
					if(do_flip && (!oldloc.Enter(tmob) || !newloc.Enter(user)))
						do_flip = FALSE
					...
					if(do_flip)
						for(var/atom/movable/obstacle in newloc)
							if(!ismob(obstacle) && !obstacle.Cross(user))
								do_flip = FALSE
								break
					if(do_flip)
						user.set_loc(newloc)
						tmob.set_loc(oldloc)
						message = "<B>[user]</B> flips over [tmob]!"
					else
						flipped_a_guy = FALSE
					user.flags = orig_user_flags
			...
			if (!flipped_a_guy)
				for (var/mob/living/M in view(1, null))
					...
					if (M in combatflipped)
						continue
					if (user.reagents?.get_reagent_amount("ethanol") > 10 && user.can_drunk_act())
